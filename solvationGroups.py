...
from rmgpy.data.solvation import * # Can narrow this down later

#from importOldDatabase import getUsername
user = 'Belinda Slakman'
...
def createDataSet(label, family, database, rxnFamily):
    """
    Create a data set from the component of the solvation kinetic `family` indicated by
    the given `label`. 
    """
    dataset = []
...
def generate(args):
    """
    Generate solvation correction group additivity values for one (or more) reaction
    ...
    # Make sure the method is valid (relevant for solvation?)
    # method = args.method
    ...
    if not trainingSetLabels:
        trainingSetLabels = [args.solvent+'/training']
    ...
    for family in families:
        for trainingSetLabel in trainingSetLabels:
            for reaction, template, entry in createDataSet(trainingSetLabel, family, rxnFamily.solvationCorrections, rxnFamily.solvationCorrections.family):
                correction = entry.data
                trainingSet.append((template, correction))
                
    # Generate the group values (implemented on the SolvationKineticsGroups class)
...
def evaluate(args):
    """
    Evaluate solvation kinetics group additivity values for one (or more) reaction
    families. The `args` parameter provides the results of parsing the 
    command-line arguments using argparse.
    """
...
def parseAndRunCommandLineArguments():
    ...
    generateParser.add_argument('--solvent', metavar='<solvent>', type=str, help='solvent for which to generate groups')
